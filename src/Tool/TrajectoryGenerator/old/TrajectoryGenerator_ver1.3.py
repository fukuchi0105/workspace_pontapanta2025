...
class TrajectoryModel:
    ...
    def simulate_lap_time(self):
        ...
        """
        # パラメータ初期値
        self.vehicle_mass = 300  #[kg]
        self.max_radius_virtual = 1000  #[m]
        self.max_horizontal_G = 1  #[G]
        self.max_accel_G = 1   # [G]
        self.max_brake_G = 1   # [G]
        self.CdA = 0.43  # [m^2]
        self.air_density = 1.225 # [kg/m-3]
        self.u_aA = 1 / 2 * self.CdA * self.air_density / 3.6**2 / 9.8    # [kgf/kph^2]
        self.u_r = 0.016 # [kgf/kgf]
        self.power = 160 #[kW]
        self.max_speed_finish = 500 #[kph]
        div = self.div_export
        ...
        for i in range(len(self.curvature_rad)-1):
            ...
            self.car_vel.append(min([np.sqrt(2 * self.car_acc[-1] * 9.8 * div + (self.car_vel[-1] / 3.6)**2) * 3.6,
                                     self.brake_speed[i]]))
...
class TrajectoryView:
    ...
    def create_buttons(self):
        ...
        self.button_calc_time = Button(plt.axes([0.02, 0.08, 0.1, 0.05]), 'Calc Time')
...
class TrajectoryController:
    ...
    def calc_time(self, event):
        ...
        print(lap_time)
        if lap_time:
            self.view.ax.legend([f"Time = {lap_time:.3f} [s]"], loc='upper right')
            self.view.redraw_all()
