...
# --- クリックイベント ---
def onclick(event):
    global current_curve, output_points, dragging_point
    if event.inaxes != ax:
        return
    if edit_mode:
        # 編集モード → 最も近い点をドラッグ対象に
        if selected_points:
            ...
            dists = [np.linalg.norm(p - click_pos) for p in selected_points]
            idx = np.argmin(dists)
            if dists[idx] < 0.2:
                dragging_point = idx
        return
    ...
    artist = ax.scatter(x, y, c='r', s=40)
...
def on_motion(event):
    ...
    if not edit_mode or dragging_point is None:
        return
    if event.inaxes != ax:
        return
    selected_points[dragging_point] = np.array([event.xdata, event.ydata])
...
# --- Fixボタン ---
def fix(event):
    global selected_points, click_artists, current_curve, output_points, fixed_curves, fixed_lines, connecting_lines
