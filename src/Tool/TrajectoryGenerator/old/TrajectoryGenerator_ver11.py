...
fixed_lines = []
fixed_points = []  # Fix済み点
scene_ids = []     # 各FixのシーンID
connecting_lines = []

edit_mode = False
dragging_info = None
scene_counter = 1  # シーン番号
...
def onclick(event):
    ...
    if edit_mode:
        # 編集モード
        all_points = [(selected_points, i, 0)] + [(fp, i, sid) for fp, sid in zip(fixed_points, scene_ids) for i in range(len(fp))]
        if not all_points:
            return
        click_pos = np.array([event.xdata, event.ydata])
        closest = None
        min_dist = float('inf')
        for plist, idx, sid in all_points:
...
# --- Fixボタン ---
def fix(event):
    global selected_points, click_artists, current_curve, output_points, fixed_curves, fixed_lines, fixed_points, scene_ids, scene_counter
    if len(output_points) == 0:
        return
    fixed_curves.append(output_points.copy())
    fixed_points.append(selected_points.copy())
    scene_ids.append(scene_counter)
    scene_counter += 1
...
# --- All Reset ---
def all_reset(event):
    global selected_points, click_artists, current_curve, output_points, fixed_curves, fixed_lines, connecting_lines, fixed_points, scene_ids, scene_counter
    ax.clear()
    selected_points.clear()
    click_artists.clear()
    output_points = []
...
def reset_prev(event):
    global selected_points, click_artists, current_curve, output_points, fixed_curves, fixed_lines, fixed_points, scene_ids
    for artist in click_artists:
        artist.remove()
    click_artists.clear()
    selected_points.clear()
...
# --- Outputボタン ---
def output(event):
    global fixed_curves, output_points, fixed_points, scene_ids
    all_points = []
    ...
    # 点情報ログ出力
    log_path = base + "_points_log.csv"
    with open(log_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['scene_id', 'x', 'y'])
        # Fix済み
        for sid, fp in zip(scene_ids, fixed_points):
            for p in fp:
                writer.writerow([sid, p[0], p[1]])
        # 現在の未Fix
        for p in selected_points:
            writer.writerow([0, p[0], p[1]])
    print(f"点情報ログを出力: {log_path}")
