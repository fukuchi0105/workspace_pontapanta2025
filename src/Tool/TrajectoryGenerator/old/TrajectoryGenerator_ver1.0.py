...
def redraw_all():
    ...
    # Editモード時の色設定
    current_color = 'orange' if edit_mode else 'r'

    # 現在の編集中
    for p in selected_points:
        ax.scatter(p[0], p[1], c=current_color, s=40)
    if len(selected_points) >= 2:
        curve = bezier_curve(np.array(selected_points))
        ax.plot(curve[:,0], curve[:,1], color=current_color, lw=2)

    # Fix済み
    for fp in fixed_points:
        for p in fp:
            ax.scatter(p[0], p[1], c=current_color, s=20)
        if len(fp) >= 2:
            curve = bezier_curve(fp)
            ax.plot(curve[:,0], curve[:,1], color=current_color, lw=2)
...
if __name__ == "__main__":
    ...
    ax.set_title('Trajectory Generator')
