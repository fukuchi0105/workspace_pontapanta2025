...
/*
[概要]
車両のIMU（ジャイロ）データとオドメトリ（車速）データを統合し、車両の「ねじれの少ない推定速度情報」を出力。

[入力]
twist_with_covariance：車両速度（共分散付き）
imu：IMU（角速度 + 共分散）

[出力]
twist_raw：フィルタリングなしの統合Twistデータ
twist_with_covariance_raw：共分散付きの統合Twistデータ
twist：補正後のTwist（低速時は角速度補正）
twist_with_covariance：補正後の共分散付きTwist

*/

// IMU共分散を最大値に正規化 (IMUのXYZ共分散の最大値を取り、3軸共通の共分散に変換)
...
// 複数フレームの車速・IMUを平均化し、TwistWithCovarianceを生成。
    ...
    const geometry_msgs::msg::TwistWithCovarianceStamped::ConstSharedPtr vehicle_twist_ptr)
{
    ...
    // IMUと同期できる場合はconcatGyroAndOdometer()で統合しpublishData()へ
...
void GyroOdometer::callbackImu(const sensor_msgs::msg::Imu::ConstSharedPtr imu_msg_ptr)
{
    ...
    // TF(Transform関数)を使いIMU座標系→base_link座標系に変換
...
// Twistの生データと補正データを各TopicへPublish
    ...
    const geometry_msgs::msg::TwistWithCovarianceStamped & twist_with_cov_raw)
{
    ...
    // 車両停止時は0を出力
    if ( std::fabs(twist_with_cov_raw.twist.twist.angular.z) < 0.01 && std::fabs(twist_with_cov_raw.twist.twist.linear.x) < 0.01) {
