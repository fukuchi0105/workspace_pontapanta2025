...
/*
[概要]
・GNSS受信データ (NavSatFix) を元に自動車の現在位置を推定
・map座標系で base_link の姿勢を算出・出力する

[入力]
/fix: sensor_msgs::msg::NavSatFix (GNSS位置情報)
/autoware_orientation: autoware_sensing_msgs::msg::GnssInsOrientationStamped (GNSS/INS姿勢)
/vehicle/status/gear_report: 車両のギア状態

[出力]
/gnss_pose: geometry_msgs::msg::PoseStamped (GNSSベースリンクの姿勢)
/gnss_pose_cov: geometry_msgs::msg::PoseWithCovarianceStamped (推定姿勢＋共分散)
/gnss_fixed: tier4_debug_msgs::msg::BoolStamped (GNSS固定状態)
...
{
    // パラメータ読込、Publisher/Subscriber登録
...
// GNSSセンサーから取得した方位角（heading）をロボットのヨー角（yaw）に変換
